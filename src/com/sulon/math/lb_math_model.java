...
public class lb_math_model {
	...
	public lb_pose2f lb_velocity_motion_model_sample( vec2f ut,
			lb_pose2f p,
			double dt,
			double var[])
	{
		double v2 = macro_functions.LB_SQR(ut.x);
		double w2 = macro_functions.LB_SQR(ut.y);
		...
		if(w != 0) {
			v_w = v/w;
			x = p.x - v_w*Math.sin(p.a) + v_w*Math.sin(p.a + w*dt);
			y = p.y + v_w*Math.cos(p.a) - v_w*Math.cos(p.a + w*dt);
			a = lb_misc_functions.lb_normalize_angle(p.a + w*dt + r*dt);
		} else {
			x = p.x + v*Math.cos(p.a);
			y = p.y + v*Math.sin(p.a);
			a = lb_misc_functions.lb_normalize_angle(p.a + r*dt);
		}
		
		lb_pose2f pos2f = new lb_pose2f(x, y, a);
	...
	public double velocity_motion(lb_pose2f pt,
			vec2f ut,
			lb_pose2f p,
			double dt,
			double var[])
	{
		double x_x = p.x - pt.x;
		double y_y = p.y - pt.y;
		double tmp0 = ((x_x*Math.cos(p.a)) + (y_y*Math.sin(p.a)));
		double tmp1 = ((y_y*Math.cos(p.a)) - (x_x*Math.sin(p.a)));
		...
		rr = Math.sqrt(macro_functions.LB_SQR(pt.x - xx) + macro_functions.LB_SQR(pt.y - yy));

		aa = lb_misc_functions.lb_minimum_angle_distance(Math.atan2(p.y - yy, p.x - xx), Math.atan2(pt.y - yy, pt.x - xx));
		v = aa/dt * rr;

		if(ut.x >= 0) {
			if(macro_functions.LB_SIGN(yy) == macro_functions.LB_SIGN(aa))
				v = macro_functions.LB_SIGN(ut.x) * Math.abs(v);   //check sign with control input
			else
				return 0;
		} else {
			if(macro_functions.LB_SIGN(yy) != macro_functions.LB_SIGN(aa))
		...
		double v2 = v*v;
		double w2 = w*w;
		double r = lb_misc_functions.lb_minimum_angle_distance(w, (pt.a - p.a)/dt);


		double p0 = lb_statistic_function.lb_pdf_normal_dist(var[0]*v2 + var[1]*w2, 0.0, v - ut.x);
		double p1 = lb_statistic_function.lb_pdf_normal_dist(var[2]*v2 + var[3]*w2, 0.0, lb_misc_functions.lb_minimum_angle_distance(ut.y, w));
	...
	public lb_pose2f lb_odometry_motion_model_sample( lb_pose2f u_pt,
			lb_pose2f u_p,
			lb_pose2f p,
			double var[])
	{
		double rot1 = lb_misc_functions.lb_minimum_angle_distance(u_p.a, Math.atan2(u_pt.y - u_p.y, u_pt.x - u_p.x));
		double tran = (u_pt.get_vec2().minus(u_p.get_vec2())).size();
		double rot2 = lb_misc_functions.lb_minimum_angle_distance(rot1, lb_misc_functions.lb_minimum_angle_distance(u_p.a, u_pt.a));
		...
		double x = p.x + ntran*Math.cos(p.a + nrot1);
		double y = p.y + ntran*Math.sin(p.a + nrot1);
		double a = lb_misc_functions.lb_normalize_angle(p.a + nrot1 + nrot2);
		lb_pose2f pos2f = new lb_pose2f(x,y,a);
		return pos2f;
	}
	 ...
	 * @return
	 */
	public double odometry_motion(lb_pose2f pt,
			lb_pose2f u_pt,
			lb_pose2f u_p,
			lb_pose2f p,
			double var[])
	{
		...
		double nrot1 = lb_misc_functions.lb_minimum_angle_distance(p.a, Math.atan2(pt.y - p.y, pt.x - p.x));
		double ntran = (pt.get_vec2().minus(p.get_vec2())).size();
		double nrot2 = lb_misc_functions.lb_minimum_angle_distance(nrot1, lb_misc_functions.lb_minimum_angle_distance(p.a, pt.a));
